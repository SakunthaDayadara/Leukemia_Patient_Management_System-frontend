...
function NurseRequestedTestTable() {
    ...
    useEffect(() => {
        ...
        const fetchRequestedTests = async (nurseId) => {
            try {
                const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/tests/requested_tests`, {
                    headers: {
                    ...
                    actions={[
                        {
                            ...
                            onClick: (event, rowData) => {
                                navigate(`/nursedashboard/test/requestedtest/${rowData.test_id}`);
...
export default NurseRequestedTestTable;
